---
name: humanizer
description: "Humanize text: strip AI-isms and add real voice. Also detect/audit whether prose reads as AI-written (signals not proof), with tiered word tables and detect/edit modes."
version: 2.5.1
...
## Signals, not proof (read this before judging SOMEONE ELSE'S text)

These patterns are statistically more common in LLM output, but **humans produce the same shapes** —
especially under deadline, in unfamiliar genres, or writing in a second language. Independent audits
found commercial AI detectors mislabel **>60% of non-native English writers** as AI (Liang et al.,
Stanford, *Patterns* 2023), open-source detectors misclassify **>70%** overall (Jabarian & Imas, BFI
2025), and adversarial paraphrase drops accuracy **~88%** (arXiv:2506.07001, 2025). So: use these as a
**signal** — for cleaning up writing (yours or on request) and for a gut-check — but **never as the
sole basis for a consequential decision** (academic integrity, hiring, publication, attribution). Pair
the signal with context: who wrote it, what genre, the writer's normal voice, what other evidence
exists. *Worth acting on; not worth ruining someone's day over.* When humanizing your own or requested
prose this caveat costs nothing — it matters the moment the task shifts to "is this AI-written?"

## Going deeper — modes, tiered vocab, triage (`references/avoid-ai-writing-folded.md`)

The 29 patterns below are the spine. For extra machinery, load
`references/avoid-ai-writing-folded.md` (folded 2026-06-23 from conorbronsdon/avoid-ai-writing, MIT):
- **Modes** — `detect` (flag only, don't rewrite — for auditing others' or published text) and `edit`
  (minimal in-place file edits, leave human passages alone) on top of the default `rewrite`.
- **Tiered vocabulary** — Tier 1 (always replace) / Tier 2 (flag in clusters) / Tier 3 (flag at
  density), a false-positive-reduction upgrade over a flat word list, with full replacement tables.
- **Newer categories** — future-narrative closers, hedge-stacked predictions, real/actual inflation,
  hashtag stuffing, bare-noun bullet lists, paste artifacts (crypto/web3/social tells the
  Wikipedia-derived list predates).
- **Severity triage (P0/P1/P2)** + **writer-side tests** (paragraph-reshuffle immunity, treadmill
  effect, TTR) for quick passes on big docs.
...
**Folded in 2026-06-23:** the tiered vocabulary tables, detect/edit modes, severity triage, newer (crypto/web3/social) pattern categories, and the "signals, not proof" epistemic guard are folded from [conorbronsdon/avoid-ai-writing](https://github.com/conorbronsdon/avoid-ai-writing) (MIT, v3.10.0) — full material + provenance in `references/avoid-ai-writing-folded.md`. The base 29 patterns and the personality/soul section remain from blader/humanizer.
